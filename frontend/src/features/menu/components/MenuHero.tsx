...
import ScrollStack, { ScrollStackItem } from "@/components/shared/scroll-stack/ScrollStack";

// Muestra platos representativos del menú apilados en ScrollStack
    ...
    <section className="relative w-full overflow-hidden bg-[#0c0a09] text-stone-100 pt-28 sm:pt-32 pb-0 flex flex-col border-b border-stone-800/60">
      ...
      <div className="relative z-10 mx-auto max-w-3xl px-6 text-center">
        ...
        <p className="mt-4 text-xs sm:text-sm text-stone-400 max-w-xl mx-auto leading-relaxed">
          Explora nuestras entradas, cortes a las brasas, bocadillos artesanales y banquetes
          preparados para llevar tu evento al siguiente nivel.
        </p>
...
>
  ...
  {MENU_HIGHLIGHTS.map((item) => (
    <ScrollStackItem key={item.title} itemClassName="menu-hero-stack-card">
      <div className="relative aspect-video w-full overflow-hidden rounded-3xl border border-stone-800/80 bg-stone-900">
        <Image
          src={item.image}
          alt={item.title}
          fill
        ...
        <span className="absolute top-5 left-5 rounded-full bg-amber-500/90 px-2.5 py-0.5 text-[10px] font-black uppercase tracking-wider text-stone-950 backdrop-blur-md">
          {item.tag}
        </span>
        ...
        <div className="absolute bottom-6 left-6 right-6 z-10">
          ...
          <h3 className="text-xl font-bold text-white leading-snug mt-0.5">{item.title}</h3>
        </div>
      </div>
    </ScrollStackItem>
  ))}
